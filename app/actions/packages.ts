...
const packageSchema = z.object({
  ...
  duration_mins: z.string().refine(v => parseInt(v) > 0, 'Duration must be greater than 0'),
...
export async function addPackage(form: {
  ...
  const { data: pkg, error } = await context.admin
    ...
    .insert({
      ...
      outfits_count: form.outfits_count ? parseInt(form.outfits_count) : null,
      edited_photos: form.edited_photos ? parseInt(form.edited_photos) : null,
